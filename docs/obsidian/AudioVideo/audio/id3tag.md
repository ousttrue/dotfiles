https://kid3.sourceforge.io/kid3_en.html#frame-list

- https://github.com/sp1ff/scribbu
- https://github.com/rstemmer/id3edit

# version

## 2.4

- https://scrapbox.io/yutsuki38/ID3(ID3v2.4)%E3%81%AB%E3%81%A4%E3%81%84%E3%81%A6

## 2.3

- http://takaaki.info/wp-content/uploads/2013/01/ID3v2.3.0J.html

## 2.2

## 1

- [mp3のタグ(ID3v1, ID3v2)を読み込む（ライブラリ不使用） #C# - Qiita](https://qiita.com/kob58im/items/681dc2262f791f64abdc)
- [【続】 mp3のタグ(ID3v1, ID3v2)を読み込む（ライブラリ不使用） ＋ DataGridViewで一覧表示する ／ コマンドライン引数に応じてID3v1タグを書き換える #C# - Qiita](https://qiita.com/kob58im/items/9a5b13c8fcd3bf287487)

# format

- https://zenn.dev/kjumanenobikto/articles/368a3b08269f8c
- https://eleken.y-lab.org/report/other/mp3tags.shtml

# tui

## python

- https://github.com/sudormrfbin/clid

- https://mutagen.readthedocs.io/en/latest/

## foobar2000

- https://foobar2000.xrea.jp/?ID3v2+tag+support+%E3%81%A4%E3%81%8B%E3%81%84%E3%81%8B%E3%81%9F

# gui

- [EasyTAG：Linuxで使える音楽ファイルのタグ編集ソフト | クロの思考ノート](http://note.kurodigi.com/linux-easytag/)


- https://www.mp3tag.de/en/
