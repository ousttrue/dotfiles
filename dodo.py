...
if IS_WINDOWS:
    from doit_lib import (
        SYNC_APPDATA_LOCAL_DIR,
        SYNC_APPDATA_ROAMING_DIR,
        APPDATA_LOCAL_DIR,
        APPDATA_ROAMING_DIR,
    )
...
if IS_WINDOWS:
    ...
    class neovim(GitBuildTask):
        ...
        actions = [
            # deps
            f"{cmake} -S %(git_dir)s/cmake.deps -B %(git_dir)s/.deps -DCMAKE_BUILD_TYPE=RelWithDebInfo -G Ninja",
            f"{cmake} --build %(git_dir)s/.deps --config RelWithDebInfo",
            # nvim
            f"{cmake} -S %(git_dir)s -B %(git_dir)s/build -DCMAKE_BUILD_TYPE=RelWithDebInfo -G Ninja",
...
if IS_WINDOWS:
    ...
    class sumneko_lua_language_server(GitBuildTask):
        ...
        actions = [
            CmdAction("cmd /C compile\\install.bat", cwd="%(git_dir)s/3rd/luamake"),
...
# ZIG_BUILD_VERSION = '3952+9e070b653'
# ZIG_BUILD_VERSION = '4198+5e0d8a435'
# ZIG_BUILD_VERSION = '4324+c23b3e6fd'
# ZIG_BUILD_VERSION = '987+a1d82352d'
ZIG_VERSION = "12"
if IS_WINDOWS:
    ZIG_ARCHIVE_NAME = f"zig-windows-x86_64-0.{ZIG_VERSION}.0-dev.{ZIG_BUILD_VERSION}"
    ...
    ZIG_ARCHIVE_NAME = f"zig-linux-x86_64-0.{ZIG_VERSION}.0-dev.{ZIG_BUILD_VERSION}"
...
def task_zig():
    return {
        "actions": [
            ...
            (mklink, [ZIG_EXTRACT_FILE.parent / "lib", ZIG_BIN.parent / "lib"]),
        ],
...
class zls(GitBuildTask):
    ...
    actions = [
        ...
        (mklink, [f"{HOME_DIR}/ghq/github.com/zigtools/zls/zig-out/bin/zls{EXE}"]),
    ]
...
class gyro(GitBuildTask):
    ...
    actions = [
        f"zig{EXE} build -Drelease-safe",
        (mklink, [f"{HOME_DIR}/ghq/github.com/mattnite/gyro/zig-out/bin/gyro{EXE}"]),
    ]
...
class wezterm(GitBuildTask):
    ...
    actions = [
        "./get-deps",
        "cargo build --release",
        f"ln -sf {HOME_DIR}/ghq/github.com/wez/wezterm/target/release/wezterm {HOME_DIR}/local/bin/wezterm",
    ]
